        ...
        for rec in self:
            ...
            if vals.get("stage_id") and rec.require_user_action_flg:
                ...
                if job_stage:
                    if job_stage[0]["project_flow"] == "end":
                        job_string = rec.project_field_string or ""
                        job_date = rec.project_field_date or ""
                        job_float = rec.project_field_float or 0.00
            ...
            if result:
                if check_project_related_field:
                    proj_dict = {}
                    proj_fld_type = rec.project_fields_type
                    if rec.project_fields and proj_fld_type:
