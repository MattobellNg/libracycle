...
class HrLeave(models.Model):
    ...
    def approval_check(self):
        ...
        for user_obj in user.leave_approvals.mapped("validating_users").filtered(
            lambda l: l.id == self.env.uid
        ):
...
class HrLeaveTypes(models.Model):
    ...
    leave_validators = fields.One2many(
    ...
    def enable_multi_level_validation(self):
        ...
        self.multi_level_validation = (
            True if (self.leave_validation_type == "multi") else False
        )
...
class HrLeaveValidators(models.Model):
    ...
    holiday_validators = fields.Many2one(
        "res.users",
...
class LeaveValidationStatus(models.Model):
    ...
    validating_users = fields.Many2one(
        "res.users",
        string="Leave Approvers",
        domain="[('share','=',False)]",
    )
